...
def random_point_on_sphere(dim=2, random_state=None):
    """
    Return a random point on the sphere, uniformly selected with respect to
    volume.
    """
...
def random_son(dim=2, random_state=None):
    ...
    SO(dim) in this case consists of the orthogonal matrices with determinant
    1. Matrices are selected uniformly with respect to Haar measure.
    """
    random_state = check_random_state(random_state)
...
class RandomCoordinateChanger(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        def create_son():
            return random_son(
                dim=self.real_dimension_, random_state=self.random_state)

        self.transforms = np.array(
            [create_son() for i in range(self.real_repeats_)])

        def choose_indices():
            return self.random_state.choice(
                indices, size=self.real_dimension_, replace=False)

        self.indices_ = np.array(
            [choose_indices() for i in range(self.real_repeats_)])
        return self
...
class CoordinateChangingDecisionTreeClassifier(CoordinateChangingDecisionTree,
                                               ClassifierMixin):
    """
    A decision tree classifier which performs a random change of
    coordinates before fitting or predicting.

    The parameters are mostly the same as sklearn's DecisionTreeClassifier,
...
class CoordinateChangingDecisionTreeRegressor(CoordinateChangingDecisionTree):
    """
    ...
    def __init__(self,
        ...
        super(RandomCoordinateForestClassifier, self).__init__(
            ...
            estimator_params=(
                ...
                "max_leaf_nodes", "min_impurity_split", "transform_repeats",
                "transform_dimension", "usecols", "random_state"),
            bootstrap=bootstrap,
            oob_score=oob_score,
            n_jobs=n_jobs,
